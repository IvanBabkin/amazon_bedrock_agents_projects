...
def create_data_qc_agent_executor():
    """Create and return the data QC agent executor."""
...
async def analyze_data_quality(pdf_bytes: bytes, excel_bytes: bytes) -> str:
    ...
    agent_executor = create_data_qc_agent_executor()
    ...
    try:
        ...
        # Fix: Ensure we return a string, handle Amazon Nova's output format
        output = result.get("output", "No output generated.")
        
        # Handle Amazon Nova's specific output format: list of content blocks
        if isinstance(output, list):
            text_parts = []
            for item in output:
                if isinstance(item, dict):
                    # Extract text from Amazon Nova content blocks
                    if 'text' in item:
            ...
            return "\n".join(text_parts) if text_parts else "No content generated."
